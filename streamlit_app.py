...
def fetch_gene_metadata_and_kegg(gene_list: list[str], organism_entrez: str,
    ...
    for i, gene in enumerate(gene_list, start=1):
        ...
        try:
            ...
            if not kegg_id:
                # Fallback via KEGG symbol search
                kegg_id = kegg_find_gene_by_symbol(gene, kegg_org_prefix)
            if not kegg_id:
    ...
    with meta_tab:
        try:
            if 'df_meta' in locals() and not df_meta.empty:
                n_total = len(df_meta)
                n_with_path = int(df_meta['KEGG_Pathways'].notna().sum())
                st.caption(f"KEGG mapping: {n_with_path}/{n_total} genes had ≥1 pathway (fallback used where needed).")
        except Exception:
            pass
