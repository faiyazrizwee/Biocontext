...
with c1: st.image("logoo.png", width=120)
with c2:
...
# ----------------------------
def _clean_gene_series(series: pd.Series) -> list[str]:
...
def load_genes_from_any(uploaded_file) -> list[str]:
    """
    Read genes from CSV/TSV/XLSX/TXT (first matching column or plain list).
    ...
    try:
        ...
        if isinstance(df, pd.DataFrame) and not df.empty:
            ...
            return _clean_gene_series(df[target_col])
    except Exception:
        pass

    # Fallback: robust TXT parsing (always rewind to start)
    try:
        try:
        ...
        if isinstance(raw, (bytes, bytearray)):
            ...
            text = str(raw)
        return _clean_gene_series(pd.Series([text]))
    except Exception:
        return []
...
def ot_target_from_symbol(symbol: str, species: str = "Homo sapiens") -> dict | None:
    # Prefer exact approvedSymbol = symbol, and human orthologs
    q = """
    query FindTarget($q: String!) {
      search(queryString: $q, entityNames: ["target"], page: {index: 0, size: 20}) {
        hits {
          id
          name
          entity
    ...
    for h in hits:
        if h.get("entity") == "target":
            appr = ((h.get("object") or {}).get("approvedSymbol") or h.get("name") or "").upper()
            if appr == symbol_upper:
                return {"id": h.get("id"), "approvedSymbol": appr}
    # else first target
    for h in hits:
        if h.get("entity") == "target":
...
@st.cache_data(ttl=3600)
def ot_diseases_for_target(ensembl_id: str, size: int = 100) -> pd.DataFrame:
    q = """
...
@st.cache_data(ttl=3600)
def ot_drugs_for_target(ensembl_id: str, size: int = 500) -> pd.DataFrame:
    # Ask for more rows; OT returns 1..4 for phase (int or str)
    q = """
    ...
    df = pd.DataFrame(out)
    if not df.empty:
        df = df.drop_duplicates(subset=["drug_id", "drug_name", "target"])
    return df
...
def chembl_find_molecule(drug_name: str) -> dict | None:
    try:
        r = SESSION.get("https://www.ebi.ac.uk/chembl/api/data/molecule.json",
                        params={"pref_name__iexact": drug_name, "limit": 1}, timeout=30)
        js = r.json()
        if js.get("page_meta", {}).get("total_count", 0) == 0:
            r = SESSION.get("https://www.ebi.ac.uk/chembl/api/data/molecule/search.json",
                            params={"q": drug_name, "limit": 1}, timeout=30)
            js = r.json()
...
def pkdb_half_life(drug_name: str) -> float | None:
    try:
        ...
        vals = []
        for p in params:
            val = p.get("value"); unit = (p.get("unit") or "").lower()
            if val is None: continue
            if "hour" in unit: vals.append(float(val))
            elif "min" in unit: vals.append(float(val)/60.0)
...
def enrich_drugs_with_pk_and_approval(df_drugs: pd.DataFrame) -> pd.DataFrame:
    ...
    # robust flags
    p_ot = _to_num(out["phase"])
    p_ch = _to_num(out["chembl_max_phase"])
    best_phase = pd.concat([p_ot, p_ch], axis=1).max(axis=1, skipna=True)
    out["approved"] = (best_phase >= 4) | out["chembl_first_approval"].notna()
    out["clinically_tested"] = (best_phase >= 1)
...
def collect_drug_suggestions(gene_to_target: dict) -> pd.DataFrame:
    ...
    if frames:
        ...
        # de-duplicate the same drug across multiple targets for cleanliness
        out = out.drop_duplicates(subset=["drug_id", "drug_name", "gene", "target"])
        return out
...
uploaded = st.file_uploader("Upload gene list (.csv, .tsv, .txt, .xlsx). If a table, I'll use the 'Gene.symbol' or 'Symbol' column.",
                            type=["csv", "tsv", "txt", "xlsx"])
manual_input = st.text_area("Or paste gene symbols here (comma, space, or newline separated):",
                            placeholder="e.g. TP53, BRCA1, EGFR, MYC")

genes_from_input: list[str] = []
if manual_input.strip():
    genes_from_input = _clean_gene_series(pd.Series([manual_input]))
...
if run_btn:
    ...
    with enrich_tab:
        ...
        if df_enrich.empty:
            ...
            st.download_button("Download enrichment CSV (counts-only)",
    ...
    with disease_tab:
        ...
        if df_dis.empty:
            ...
            show = agg.copy(); show.insert(0, "#", range(1, len(show) + 1))
            st.dataframe(show.reset_index(drop=True), use_container_width=True, hide_index=True)
            st.download_button("Download disease associations (per gene)",
                               data=df_dis.to_csv(index=False).encode("utf-8"),
                               file_name="gene_disease_links_opentargets.csv", mime="text/csv")
            st.download_button("Download disease summary (aggregated)",
    ...
    with drug_tab:
        ...
        if df_drugs.empty:
            ...
            # Rank safely
            df_drugs["phase_rank"] = _to_num(df_drugs["phase"]).fillna(0).astype(int)
            df_drugs["chembl_phase_rank"] = _to_num(df_drugs["chembl_max_phase"]).fillna(0).astype(int)
            df_drugs = df_drugs.sort_values(["approved", "phase_rank", "chembl_phase_rank", "drug_name"],
            ...
            drug_sum = (df_drugs.groupby(["drug_id", "drug_name"]).agg(
                ...
                max_phase=("phase", lambda s: _to_num(s).max()),
                ...
                chembl_max_phase=("chembl_max_phase", lambda s: _to_num(s).max()),
                chembl_atc=("chembl_atc", "first"),
                chembl_roa=("chembl_roa", "first"),
                pk_half_life_h=("pk_half_life_h", "median"),
            ).reset_index())

            drug_sum["phase_rank"] = _to_num(drug_sum["max_phase"]).fillna(0).astype(int)
            drug_sum["chembl_phase_rank"] = _to_num(drug_sum["chembl_max_phase"]).fillna(0).astype(int)
            ...
            with colf1: only_approved = st.checkbox("Only approved drugs", value=True)
            with colf2: include_clinical = st.checkbox("Include clinically tested (Phase ≥1)", value=True)
            with colf3: show_top_n = st.number_input("Show top N", min_value=5, max_value=200, value=50, step=5)

            # Filtering with graceful fallback
            filt = drug_sum.copy()
            if only_approved:
                tmp = filt[filt["approved"] == True]
                if tmp.empty and include_clinical:
                    st.info("No approved drugs; showing clinically tested (Phase ≥1).")
                    tmp = filt[(filt["clinically_tested"] == True)]
                filt = tmp
            elif include_clinical:
                filt = filt[(filt["approved"] == True) | (filt["clinically_tested"] == True)]

            st.markdown("**Recommended drugs (ranked)**")
            st.dataframe(filt.head(int(show_top_n)).reset_index(drop=True),
                         use_container_width=True, hide_index=True)

            st.download_button("Download drug suggestions (per target, with PK/PD + approval)",
                               data=df_drugs.to_csv(index=False).encode("utf-8"),
                               file_name="drug_suggestions_per_target_enriched.csv", mime="text/csv")
            st.download_button("Download drug suggestions (aggregated, with PK/PD + approval)",
                               data=drug_sum.to_csv(index=False).encode("utf-8"),
                               file_name="drug_suggestions_aggregated_enriched.csv", mime="text/csv")
            ...
            try:
                if 'df_dis' in locals() and not df_dis.empty:
                    ...
                    if 'df_drugs' in locals() and not df_drugs.empty:
                        ...
                        tmp['phase_rank'] = _to_num(tmp['phase']).fillna(0).astype(int)
                    ...
                    if drugs_set and 'df_drugs' in locals() and not df_drugs.empty:
                        ...
                        for _, row in tmp.iterrows():
                            ...
                            if t is not None:
                                val = int((_to_num(row.get('phase')) or 0))
